...
public class TournamentSelection implements SelectionOperator {
    ...
    public int[] select(int[] population) throws EvolutionaryAlgorithmException {
        ...
        if (population != null && population.length > 0) {
            if(tournamentSize != population.length){ //El tournament size y el population.length deben coincidir
                throw new EvolutionaryAlgorithmException("El tamaño de tournamentSize debe ser igual que population.length");
            }
